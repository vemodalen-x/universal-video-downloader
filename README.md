...
3. 播放列表可按住 `Ctrl` 或 `Shift` 多选，也可点击“全选列表”；普通链接会选择推荐项。
4. 在右侧确认保存位置、质量策略和字幕选项；“格式详情”可查看当前条目的技术参数。
5. 点击“开始下载”或“下载选中项”，队列会顺序执行并为每个条目建立独立任务记录。
...
- 单次播放列表解析最多展示 50 个条目，避免超大列表阻塞桌面客户端。
