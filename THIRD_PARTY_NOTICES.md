...
| Pillow | HPND | Brand asset generation and validation tooling |
| PyInstaller | GPL-2.0-or-later with bootloader exception | Windows packaging |

Versions included in a release are determined by `requirements.txt`, `requirements-build.txt`, and the release build log. Full license texts and source information are available from each upstream project and its installed package metadata.
